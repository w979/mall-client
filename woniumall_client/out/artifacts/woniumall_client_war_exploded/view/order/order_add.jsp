<%@ page language="java" pageEncoding="UTF-8"%>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>

<html>
<head>
<meta charset="utf-8" />
<base href="${base}/" />
<title>核对订单信息-${site}</title>
<jsp:include page="/view/base.jsp" />
	<script>
		//
		// function isform(){
		// 	let count=0;
		// 	let deliveryTypes = document.getElementsByName("deliveryType");
		// 	if (deliveryTypes[i].value==null && deliveryTypes[i].value==""){
		// 		console.log(deliveryTypes[i].value)
		// 		count++;
		// 	}
		// 	console.log(count)
		// 	if (count == deliveryTypes.length){
		// 		alert("请选择运送方式！")
		// 		return false;
		// 	}else {
		// 		return true;
		// 	}
		// }
	</script>
</head>
<body class="second">
	<div class="brand_list container_2">
		<jsp:include page="/view/header.jsp" />
		<div class="wrapper clearfix">
			<div class="position mt_10">
				<span>您当前的位置：</span> <a href=""> 首页</a> » 填写核对订单信息
			</div>
			<div class="myshopping m_10">
				<ul class="order_step">
					<li class="current_prev"><span class="first"><a
							href='javascript:window.history.go(-1);'>1、查看购物车</a></span></li>
					<li class="current"><span>2、填写核对订单信息</span></li>
					<li class="last"><span>3、成功提交订单</span></li>
				</ul>
			</div>

			<form action='ucenter/order?opr=submitCart' method='post' name='order_form' onsubmit="return isform()">
				<input type="hidden" name="tips" value="${tip}">
				<div class="cart_box m_10">
					<div class="title">填写核对订单信息</div>
					<div class="cont">

						<!--地址管理 开始-->
						<div class="wrap_box">
							<h3>
								<span class="orange">收货人信息</span>
							</h3>
							<!--收货表单信息 开始-->
							<div class="prompt_4 m_10" id='address_often'>
								<strong>常用收货地址</strong>
								<ul class="addr_list">
									<c:forEach items="${addressList}" var="address">
										<li><label>
											<input class="radio" name="addressCart"
												type="radio" value="${address.accept},${address.province}${address.city}${address.area}${address.address},${address.telphone}"
												<c:if test="${address.type eq 'y'}">checked</c:if> />${address.accept}&nbsp;&nbsp;&nbsp;&nbsp;${address.province}
												${address.city} ${address.area} ${address.address} </label></li>
									</c:forEach>
								</ul>
							</div>
						</div>
						<!--地址管理 结束-->
						<div class="wrap_box" id="deliveryBox">
							<h3>
								<span class="orange">配送方式</span>
							</h3>
							<!--配送修改 开始-->
							<table width="100%" class="border_table m_10" id="delivery_form"
								style="display: table;">
								<colgroup>
									<col width="180px">
									<col>
								</colgroup>
								<tbody id="deliveryFormTrBox">
									<tr>
										<th><label><input type="radio" name="deliveryType" onclick="fun(${totalMoney})"
												paytype="0" alt="20.00" value="快递" >快递</label></th>
										<td>直接由第三方物流公司配送 运费：￥20.00&nbsp;&nbsp;</td>
									</tr>
									<tr>
										<th><label><input type="radio" name="deliveryType" onclick="fun(${totalMoney})"
												paytype="0" alt="10.00" value="EMS">EMS</label></th>
										<td>运费：￥10.00&nbsp;&nbsp;</td>
									</tr>
								</tbody>
								<tfoot>
									<tr>
										<th>指定送货时间：</th>
										<td><label class="attr"><input type="radio"
												name="deliveryTime" checked="checked" value="任意">任意</label>
											<label class="attr"><input type="radio"
												name="deliveryTime" value="周一到周五">周一到周五</label> <label
											class="attr"><input type="radio" name="deliveryTime"
												value="周末">周末</label></td>
									</tr>
								</tfoot>
							</table>
							<!--配送修改 结束-->
						</div>
						<!--配送方式 结束-->

						<!--支付方式 开始-->
						<div class="wrap_box" id='paymentBox'>
							<h3>
								<span class="orange">支付方式</span>
							</h3>

							<table width="100%" class="border_table" id='payment_form'>
								<col width="200px" />
								<col />

								<tr>
									<th><label><input class="radio" name="payType"
											alt="0" title="支付宝" type="radio" value="2" checked/>支付宝</label></th>
									<td>支付手续费：￥0</td>
								</tr>
								<tr>
									<th><label><input class="radio" name="payType"
											alt="0" title="货到付款" type="radio" value="1" />货到付款</label></th>
									<td>支付手续费：￥0</td>
								</tr>
								<tr>
									<th><label><input class="radio" name="payType"
													  alt="0" title="微信" type="radio" value="3" />微信</label></th>
									<td>支付手续费：￥0</td>
								</tr>
							</table>
						</div>
						<!--支付方式 结束-->
						<!--购买清单 开始-->
						<div class="wrap_box">
							<h3>
								<span class="orange">购买的商品</span>
							</h3>

							<table width="100%" class="cart_table t_c">
								<col width="115px" />
								<col />
								<col width="80px" />
								<col width="80px" />
								<col width="80px" />

								<thead>
									<tr>
										<th>图片</th>
										<th>商品名称</th>
										<th>单价</th>
										<th>数量</th>
										<th class="last">小计</th>
									</tr>
								</thead>

								<tbody>
									<c:set var="totalMoney" value="0"></c:set>
									<c:forEach items="${cartList}" var="orderDetail" varStatus="s">
										<input type="hidden" name="goodsid" value="${orderDetail.goods.id}">
										<input type="hidden" name="nums" value="${orderDetail.nums}">
										<input type="hidden" name="price" value="${orderDetail.price}">
										<tr>
											<td><img src="${orderDetail.goods.image}"
												width="66px" height="66px" alt="${orderDetail.goods.name}"
												title="${orderDetail.goods.name}" /></td>
											<td class="t_l"><a href="" class="blue">${orderDetail.goods.name}</a></td>
											<td>￥<b>${orderDetail.price}</b></td>
											<td>${orderDetail.nums}</td>
											<td>￥<b class="red2">${orderDetail.price*orderDetail.nums}</b></td>
											<c:set var="totalMoney" value="${totalMoney+orderDetail.price*orderDetail.nums }"></c:set>
										</tr>
									</c:forEach>
									<!-- 商品展示 结束-->

								</tbody>
							</table>
						</div>
						<!--购买清单 结束-->

					</div>
				</div>

				<!--金额结算-->
				<div class="cart_box" id='amountBox'>
					<div class="cont_2">
						<strong>结算信息</strong>
						<div class="pink_box">
							<p class="f14 t_l">
								商品总金额：<b>${totalMoney}</b> + 运费总计：<b id='delivery_fee_show'>0.00</b>
							</p>
						</div>
						<hr class="dashed" />
						<div class="pink_box gray m_10">
							<table width="100%" class="form_table t_l">
								<col width="220px" />
								<col />
								<col width="250px" />
								<tr>
									<td class="t_r"><b class="price f14">应付总额：<span
											class="red2">￥<b id='final_sum'>${totalMoney}</b></span>元
									</b></td>
								</tr>
							</table>
						</div>
						<p class="m_10 t_r">
						   <input type="hidden" id="totalMoney"  name="totalMoney" value="${totalMoney}">
							<input type="submit" class="submit_order" />
						</p>
					</div>
				</div>

			</form>

		</div>
		<jsp:include page="/view/footer.jsp" />
	</div>
<script>


	//计算总金额
	function fun(){
		let money = '${totalMoney}';
		let deliveryTypes = document.getElementsByName("deliveryType");
		for (let i=0;i<deliveryTypes.length;i++){
			if (deliveryTypes[i].checked == true && deliveryTypes[i].value=='快递'){
				document.getElementById("delivery_fee_show").innerText='20.00';
				let moneys = document.getElementById("final_sum");
				moneys.innerText=parseFloat(money)+20.00;
				document.getElementById("totalMoney").setAttribute("value",moneys.innerText)
			}else if (deliveryTypes[i].checked == true && deliveryTypes[i].value=='EMS'){
				document.getElementById("delivery_fee_show").innerText='10.00';
				let moneys = document.getElementById("final_sum");
				moneys.innerText=parseFloat(money)+10.00;
				document.getElementById("totalMoney").setAttribute("value",moneys.innerText)
			}
		}
	}


	let msg='${msg}';
	if (msg != null && msg != ""){
		alert(msg)
	}
	<% request.setAttribute("msg", null); %>
</script>
</body>
</html>
